...
//
// FEATURES:
// * Works with many different ultrasonic sensor models: SR04, SRF05, SRF06, DYP-ME007 & Parallax PING)))Ã¯Â¿Â½.
...
//   don't use a library).
//
// 07/14/2012 v1.4 - Added support for the Parallax PING)))Ã¯Â¿Â½ sensor. Interface
...
class NewPing {
	public:
		...
		NewPing(uint8_t echo_pin, int max_cm_distance = MAX_SENSOR_DISTANCE);
		unsigned int ping();
		...
		void ping_timer(unsigned long maxTime);
		boolean check_timer();
